...
深度优先遍历（Deepth-First-Search, DFS），可以与[树的先序遍历](./第5章%20树与二叉树#树的先序遍历)相联系
...
[DFS算法](#6.3.2%20深度优先遍历DFS)实现拓扑排序（综合题第9题）

DFS 本身就是按**完成时间递增**的顺序结束递归的，因此只要在退栈时**从数组末尾往前**填写顶点，就等价于把 `finishTime` 从大到小排序，无需再额外调用一次排序算法（省掉 $O(|V|\log|V|)$，总复杂度仍为 $O(|V|+|E|)$）。
...
void DFS(Graph G,int v){		//从顶点v出发，深度优先遍历图G
	...
	for(w=FirstNeighbor(G,v); w>=0; w=NextNeighbor(G,v,w)){
		if(!visited[w])			//w是v的尚未访问的邻接顶点
			DFS(G,w);			//注意是DFS(G,w)，不是DFS(G,v)
	}
	print[idx--]=v;				//完成时间越晚，在拓扑序列中越靠前
}

void DFSTopologicalSort(Graph G){
	for(v=0; v<G.vexnum; v++)	//初始化标记数组
		visited[v]=FALSE;
	idx = G.vexnum-1;
	for(v=0; v<G.vexnum; v++){
...
>[!warning] 
> DFS 法本身**不检查回路**：图中有环时它照样能输出一个序列，只是该序列不是拓扑序列。要保证结果正确，须先用下文的方法判环。
...
代码实现（栈实现）：删去出度为 0 的顶点后，需要把它的**前驱**的出度减 1，因此必须使用**逆邻接表**（边表中存放的是指向该顶点的顶点）。

```c
// G 用逆邻接表存储：G.vertices[i] 的边表中存放所有指向 i 的顶点
// outdegree[] 为各顶点出度（用原邻接表统计各边表长度即可得到）
bool ReverseTopologicalSort(Graph G){
	...
	while(!IsEmpty(S)){
		Pop(S,i);
		print[count++]=i;			//记录逆拓扑序列
		for(p=G.vertices[i].firstarc; p; p=p->nextarc){
			v= p->adjvex;			//v 是 i 的前驱
			if((--outdegree[v])==0)	Push(S,v);
		}
	}
	if(count<G.vexnum)	return false;	//排序失败，有向图中存在回路
		else return true;
...
* 若只有正邻接表而没有逆邻接表，则每轮都要扫描整张表寻找出度为 0 的顶点，退化为 $O(|V|^{2})$；邻接矩阵同理（扫描某行是否全 0）。
...
**为什么不对**：考虑菱形图 `a→b`、`a→c`、`b→d`、`c→d`。从 a 出发走 a→b→d，回溯后再走 a→c，此时又碰到已访问的 d，但图中根本没有环——这条边是**横叉边/前向边**。所以“碰到 visited 的顶点”只能说明有别的路径先到过它。关键在于区分：碰到的顶点是**已经彻底处理完毕**，还是**仍在当前这条递归路径（栈）上**——只有后者（**后向边**）才意味着能绕回来，即存在回路。

**方法一：Kahn 算法（入度法）判环——最常用**

即上文的 `TopologicalSort`：执行完后若 `count < G.vexnum`，说明剩余顶点入度均不为 0 却又无法输出，它们必定相互牵制构成回路。

> **有向图存在拓扑序列 $\iff$ 该图无环**（考研标准答法）
...
void DFS(Graph G,int v){
	color[v]=1;								//入栈，染灰
	for(w=FirstNeighbor(G,v); w>=0; w=NextNeighbor(G,v,w)){
		if(color[w]==1){					//指回栈中顶点 → 后向边 → 有环
			hasCycle=TRUE;  return;
		}
		else if(color[w]==0)  DFS(G,w);
	}
	color[v]=2;								//退栈，染黑
}

bool HasCycle(Graph G){
	for(v=0; v<G.vexnum; v++)  color[v]=0;
	hasCycle=FALSE;
	for(v=0; v<G.vexnum; v++)
		if(color[v]==0 && !hasCycle)  DFS(G,v);
	return hasCycle;
}
```

回到菱形的例子：第二次碰到 d 时 d 已是**黑色**（早已完成），不算环；只有碰到**灰色**顶点才是环。时间复杂度 $O(|V|+|E|)$。

**补充：无向图判环**（考点略有不同）

* DFS 时若碰到已访问的邻接点 w，且 w **不是** v 的父结点，则存在环（否则只是沿原边走回去）。
* 或直接数边：连通无向图中若 $|E| \ge |V|$ 则必有环；无环连通图（树）恰有 $|V|-1$ 条边。
* 或用并查集：加边时若两端点已属同一集合，则成环。
