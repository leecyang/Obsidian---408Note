...
- **树**：$n~(n\geqslant 0)$ 个结点的有限集，是一种递归定义的数据结构，适合表示具有层次关系的数据。当 $n=0$ 时，称为**空树**。
- 非空树的特性：有且只有一个根结点
- **根结点**：没有父结点的结点。除了根结点外，树中任何结点都有且仅有一个前驱。
- **分支结点**：度不为 0（有子结点）的结点；
- **叶子结点**：度为 0（没有子结点）的结点。
- **子树**：当 $n>1$ 时，除根结点外的其余结点可分为 $m~(m>0)$ 个互不相交的有限集合，每个集合本身又是一棵树，称为根结点的子树。

## 5.1.2 基本术语

* 结点之间的关系描述
	+ 双亲：结点的直接前驱，即向上一层与该结点相连的结点。
	+ 孩子：与双亲的定义相对应
	+ 祖先：根结点到该结点的路径上经过的所有结点（不含该结点本身）都是该结点的祖先。
	+ 子孙：与祖先的定义相对应
	+ 兄弟：有共同双亲结点的结点。
	+ 堂兄弟：双亲不是同一结点但处于同一层的两个结点互为堂兄弟。
* 结点、树的属性描述
	+ **结点的度**：一个结点的孩子（分支）个数。
	+ **树的度**：树中所有结点度数的最大值。
	...
	+ 树的路径长度：指树根到每个结点的路径长度的总和，根到各结点的路径长度的最大值等于树的高度减 1。
* 有序树vs无序树
...
| 最小高度为 $\lceil \log_{m}(n(m-1)+1)\rceil$，<br>最大高度为 $n-m+1$ | 最小高度为 $\lceil \log_{m}(n(m-1)+1)\rceil$^[此处可以理解为 $\dfrac{m^{h-1}-1}{m-1}<n\leqslant \dfrac{m^{h}-1}{m-1}$]，<br>最大高度为 $n$ |
...
* 对于完全二叉树，设度为0、1和2的结点个数分别为 $n_{0}$、$n_{1}$ 和 $n_{2}$，则 $n_{1}=0$ 或 $1$，故可由结点数 $n$ 推出 $n_{0}$、$n_{1}$ 和 $n_{2}$。
* 含有 $n$ 个结点的二叉树共有 $\dfrac{\mathrm{C}_{2n}^{n}}{n+1}=\dfrac{(2n)!}{n!(n+1)!}$ 种不同形态，即：[卡特兰数](第3章%20栈和队列#栈的常见题型)

## 5.2.2 几种特殊的二叉树

**满二叉树**（形态特殊）：高度为 $h$ ，且含有 $2^{h}-1$ 个结点
* 只有最后一层有叶子结点 ，不存在度为1的结点
* 按层序从 $1$ 开始编号，结点 $i$ 的左孩子为 $2i$，右孩子为 $2i+1$，结点 $i$ 的父结点为 $\left \lfloor i/2 \right \rfloor$（如果有的话）
```tikz
\begin{document}
...
int getParent(TreeNode t[], int length, int index){
	if (index == 1) return -1;//根结点没有父结点
	int parent = index / 2;
	if (isEmpty(t, length, parent)) return -1;//父结点为空
	return parent;
}
```
...
### 二叉树的链式存储
 ...
 $n$ 个结点→ $2n$ 个指针域，其中有 $n-1$ 个指针指向孩子结点（除根结点外，每个结点都被一个指针指向），所以一共有 $2n-(n-1)=n+1$ 个空链域，可以用于构造[线索二叉树](#5.3.2%20线索二叉树)
...
* 寻找指定结点 $p$ 的左/右孩子较为方便
* 寻找指定结点 $p$ 的父结点时，只能从根结点开始遍历，所以也可以<u>使用三叉链表，即在结构体中再定义一个指针，指向父结点</u>
...
* 后序序列（左右**根**） + 中序序列（左**根**右）： 后序序列的最后一个结点一定是二叉树的根结点，该结点将中序序列分为两个子序列，分别对应根结点的左子树和右子树。再在子序列中递归拆分，即可得到完整的二叉树。
* 层序序列（**根**左右） + 中序序列（左**根**右）：层序序列的第一个结点一定是二叉树的根结点，该结点将中序序列分为两个子序列，分别对应根结点的左子树和右子树；每个子序列中最先出现在层序序列中的结点就是该子树的根结点，再递归拆分，即可得到完整的二叉树。
...
* 采用非递归的后序遍历时，栈中的所有结点都是当前访问结点的祖先，据此可由子孙结点找到祖先结点。 #考前记一记 
...
为了区分 `lChild` 和 `rChild` 指向的是左右孩子还是前驱或后继结点，需要增加 `tag` 作为标识：
* `tag == 0`：指针指向结点的孩子
* `tag == 1`：指针指向结点的前驱/后继
...
后序线索二叉树无法直接找到后序后继，所以如果要通过后序线索二叉树遍历二叉树的话，需要栈的辅助（来找父结点）
...
* 删（叶子结点）：① 将其双亲指针域设置为`-1`（存在空数据，遍历更慢）；②用后面的数据填补；（需要更改结点数`n`）
...
孩子表示法（顺序+链式）：顺序存储各结点，每个结点中保存孩子链表的头指针
...
* 树↔二叉树：本质是用[二叉链表存储树](#孩子兄弟表示法)，左指针指向第一个孩子，右指针指向下一个兄弟
...
若树非空，先依次对每棵子树进行后根遍历，最后访问根结点；（和与之对应二叉树的[后序遍历序列](#二叉树的后序遍历)相同）
...
* **先序遍历**：等同于依次对各棵树进行**先序遍历**；也可以先转换成与之对应的二叉树，对二叉树进行**先序遍历**；
	* 若森林非空，则按如下规则进行遍历：
	* 访问森林中第一棵树的根结点
	* 先序遍历第一棵树中根结点的子树森林
	* 先序遍历除去第一棵树之后剩余的树组成的森林
* **中序遍历**：等同于依次对各棵树进行**后序遍历**；也可以先转换成与之对应的二叉树，对二叉树进行**中序遍历**；
	* 若森林非空，则按如下规则进行遍历：
	* 中序遍历第一棵树中根结点的子树森林
	* 访问森林中第一棵树的根结点
...
哈夫曼树的定义：在含有 $n$ 个带权叶结点的二叉树中，带权路径长度（WPL）最小的二叉树称为哈夫曼树，也称为**最优二叉树**
...
2. 构造一个新结点，从 $F$ 中选取两棵根结点权值最小的树作为新结点的左右子树，并且将新结点的权值置为左右子树上根结点的权值之和
...
* 含 $n$ 个叶结点的哈夫曼树，其结点总数为 $2n-1$ 
* <u>哈夫曼树中只有度为 $0$ 和度为 $2$ 的结点</u>
* 哈夫曼树并不唯一，但WPL必然相同且为最优
* 哈夫曼树中上层结点的权值不小于下层结点的权值

多叉（$m>2$）情形：设 $r=(n_{0}-1)\bmod(m-1)$，若 $r\neq 0$，则需添加 $(m-1)-r$ 个权值为 $0$ 的虚结点，若 $r=0$ 则无需添加 
...
哈夫曼编码：由哈夫曼树得到哈夫曼编码，字符集中每一个字符作为一个叶子结点，各个字符出现的频度作为结点的权值，构造哈夫曼树（哈夫曼树同样不唯一）。
...
* 思路：<u>在每次`Union`操作构建树的时候，尽可能让树不长高</u>
	...
	* 用根结点存储值的绝对值<!--其实就是相反数，根结点S[]为负数-->表示树中的结点总数
