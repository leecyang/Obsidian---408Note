...
void InsertSort(int A[], int n){	//A中共n个数据元素
	...
	for(i=1; i<n; i++){//A[0]不需要排
		if(A[i] < A[i-1]){		//A[i]关键字小于前驱
			...
			A[j+1] = temp;		//复制到插入位置
			//上一步中，因为A[j]<=temp或j<0，所以跳出循环，所以这里是j+1
		}
	}
}
```

* 带[“哨兵”](./第7章%20查找#顺序查找%20-%20哨兵方式) ，优点：不用每轮循环都判断 $j\geqslant0$ 
...
* 先将待排序表分割成若干形如 $L[i,i+d,i+2d,\cdots,i+kd]$ 的特殊子表，<!--共d个子表-->对各个子表分别 进行**直接插入排序**。缩小增量 $d$ ，（建议每次增量缩小一半）重复上述过程，直到 $d=1$ 为止
...
```c
//希尔排序（逐子表处理）
//思想：每一趟按增量d划分若干子表，然后依次对每个子表进行直接插入排序

void ShellSort(ElemType A[], int n){
	int d, i, j, k;
	//A[0]作为暂存单元
	for(d=n/2; d>=1; d=d/2){		//步长递减
		//依次处理每一个子表，第k个子表的第一个元素为A[k]
		for(k=1; k<=d; ++k){
			//对子表A[k],A[k+d],A[k+2d]...进行直接插入排序
			for(i=k+d; i<=n; i+=d){
				if(A[i] < A[i-d]){
					A[0]=A[i];		//暂存待插入元素
					for(j=i-d; j>=k && A[0]<A[j]; j-=d){
						A[j+d]=A[j];	//子表内元素后移
					}
					A[j+d]=A[0];	//插入
				}
...
* 适用性：希尔排序基于随机存取的特性，只能使用顺序表不能使用链表实现
## 8.2.4 本节习题精选
...
\end{document}
```
取 $49_{1}$ 作为枢轴，`high`指针向前遍历，直到`A[high]<pivot`，再将`high`所指元素移动到`low`所指位置，`low`指针开始向后遍历。
```tikz
\usetikzlibrary{fit, backgrounds, shadows}
\begin{document}
	\begin{tikzpicture}[scale=2, ultra thick,
...
\end{document}
```
`low`指针不断向后遍历，直到`A[low]>pivot`，再将`low`所指元素移动到`high`所指位置，`high`指针开始向前遍历
```tikz
\usetikzlibrary{fit, backgrounds, shadows}
\begin{document}
	\begin{tikzpicture}[scale=2, ultra thick,
...
快速排序 #算法题
```c
//个人理解：取第一个元素作为枢轴（顺便空出一个位置来，且空位置由low指针指向），扫描到小于枢轴的元素，就把这个元素移动到空位置，然后开始扫描大于枢轴的元素（空位置由high指针指向）。循环往复
...
* 空间复杂度：$O(1)$ 
* 时间复杂度：$O(n^{2})$。无论有序、逆序、乱序，都需要$n-1$次的处理，总共需要对比关键字 $(n-1)+(n-2)+\cdots +1 = n(n-1)/2$ 次，元素交换次数 $\leqslant n-1$
...
> 可以将堆视为一棵 [[第5章 树与二叉树#^complete-binary-tree|完全二叉树]](✔)
> 可以将堆视为一棵 [二叉排序树](第7章%20查找#7.3.1%20二叉排序树(BST)) (✖)
...
\end{document}
```
依次调整分支结点9、78、17、53，其中调整结点53的过程如下：
```tikz
\usetikzlibrary{positioning}
\begin{document}
...
void HeapSort(int A[], int len){
	...
	for(int i=len; i>1; i--){		//i=len指向当前待排序序列的最后一个元素（堆底元素）
		//n-1趟的交换和建堆过程
...
* 第 $i$ 层最多有$2^{i-1}$个结点，而只有第$1\sim (h-1)$层的结点才有可能需要下坠调整，故==关键字对比次数 $\leqslant 4n$==
...
* 插入元素：对于小根堆，先将新元素直接放在表尾，再与父结点对比，若新元素比父结点更小，则将两者互换。<u>新元素就这样一路上升</u>，直到无法继续上升为止
* 删除元素：对于小根堆，将被删除的元素用堆底元素替代，然后让该元素<u>不断下坠</u>，直到无法继续下坠为止
...
基数排序(Radix Sort)算法思想：假设长度为 n 的线性表中每个结点 $a_{j}$ 的关键字由 $d$ 元组 $(k_{j}^{d-1},k_{j}^{d-2},\cdots,k_{j}^{1},k_{j}^{0})$ 组成，其中 $0\leqslant k_{j}^{i} \leqslant r-1$，$r$ 为基数。基数排序得到递减序列的过程如下：

* 初始化：设置 $r$ 个空队列 $Q_{r-1},\cdots,Q_{1},Q_{0}$
* 按照各个关键字位权重递增的次序（个、十、百）对 $d$ 个关键字位分别做分配和收集
	* 分配：顺序扫描各个元素，若当前处理的关键字位 $=x$ ， 则将元素插入$Q_{x}$队尾
	* 收集：把 $Q_{r-1},\cdots,Q_{1},Q_{0}$ 各个队列中的结点依次出队并链接
...
\end{document}
```
第二趟以第一趟的结果为基础，以十位分配，十位相同的按个位递减排序
```tikz
\usetikzlibrary{positioning}
\newcommand{\drawMyVar}[2]{
	\pgfmathsetmacro{\myvar}{#1}
...
\end{document}
```
第三趟以第二趟的结果为基础，以百位分配，百位相同的按十位递减排序
```tikz
\usetikzlibrary{positioning}
\newcommand{\drawMyVar}[2]{
	\pgfmathsetmacro{\myvar}{#1}
...
| 排序算法 | 一趟排序确定一个元素最终位置 | 元素比较次数与原始序列无关 | 排序趟数与原始序列有关 | 元素移动次数与原始序列无关 | 只适用于顺序存储 | 适合并行 |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| [直接插入排序](#8.2.1%20直接插入排序) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| [折半插入排序](#8.2.2%20折半插入排序) | ❌ | ✅ | ❌ | ❌ | ✅ | ❌ |
| [希尔排序](#8.2.3%20希尔排序) | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ |
| [冒泡排序](#8.3.1%20冒泡排序) | ✅ | ❌ | ✅ | ❌ | ❌ | ❌ |
| [快速排序](#8.3.2%20快速排序) | ✅ | ❌ | ✅ | ❌ | ✅ | ✅ |
| [简单选择排序](#8.4.1%20简单选择排序) | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| [堆排序](#8.4.2%20堆排序) | ✅ | ❌ | ❌ | ❌ | ✅ | ❌ |
| [2路归并排序](#8.5.1%20归并排序) | ❌ | ✅ | ❌ | ❌ | ❌ | ✅ |
| [基数排序](#8.5.2%20基数排序) | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ |
...
> 这一类算法的特点是：
>
> **每完成一次主要操作，就有一个元素已经不会再移动。**
>
> 包括：
>
> - 冒泡排序：
>   - 每趟把当前最小（或最大）元素移动到最终位置。
> - 快速排序：
>   - 每次划分后，pivot 已经位于最终位置。
> - 简单选择排序：
>   - 每趟选择最小元素，与当前位置交换。
> - 堆排序：
>   - 每趟将堆顶元素交换到末尾，最大元素确定位置。
>
> 记忆：
>
> > **交换、选择找位置，四个排序每趟定一个。**
...
> | 算法 | 原因 |
> | :--: | :-- |
> | 折半插入排序 | 需要访问中间位置 `A[mid]` |
> | 希尔排序 | 需要访问间隔为 `d` 的元素 `A[i+d]` |
> | 快速排序 | 依赖 `low/high` 双指针扫描 |
> | 堆排序 | 利用数组下标表示完全二叉树 |
>
> 记忆：
>
...
* 优化思路1：使用多路归并来增加路数 $k$ 
	* 代价1：需要增加相应的输入缓冲区
	* 代价2：每次从 $k$ 个归并段中选一个最小元素需要进行 $k-1$ 次关键字对比——可以通过“[败者树](#8.7.3%20多路平衡归并与败者树)”减少关键字对比次数
* 优化思路2：减少初始归并段数量 $r$ ，即增加初始归并段长度

按照本节介绍的方法生成的初始归并段，若共有 $N$ 个记录，内存工作区可以容纳 $L$ 个记录，则初始归并段数量 $r=\left \lceil {N/L} \right \rceil$ ，可以通过“[置换-选择排序](#8.7.4%20置换-选择排序)”进一步减少初始归并段数量
...
* $k$ 路归并的败者树只需要定义一个长度为 $k$ 的数组即可，而叶子结点是虚拟的
...
土办法：用于内部排序的内存工作区WA可容纳 l 个记录，则每个初始归并段也只能包含 l 个记录，若文件共有 $n$ 个记录，则初始归并段的数量 $r=\left \lceil {n/l} \right \rceil$ ——由内存区大小决定

置换-选择排序流程：现有待排序文件FI、内存工作区WA、初始归并输出文件FO
1. 从 `FI` 输入 l 个记录到工作区 `WA`
2. 从 `WA` 中选出其中<u>关键字取最小值的记录</u>，作为 `MINIMAX` 记录 
3. 将 `MINIMAX` 记录输出到 `FO` 中去
4. 若 `FI` 不空，则从 `FI` 输入下一个记录到 `WA` 中
5. 从 `WA` 中所有关键字比 `MINIMAX` 记录的关键字大的记录中选出最小关键字记录，作为新的 `MINIMAX` 记录
6. 重复3~5，直到 `WA` 内的关键字都比 `MINIMAX` 更小，则该归并段在此截止，得到一个初始归并段
7. 重复2~6，直到 `WA` 为空，由此得到全部的初始归并段
...
* 归并段的神秘性质：<u>初始归并段所占块数各不相同，两两合并进行二路归并</u>
...
* 构造[哈夫曼树](./第5章%20树与二叉树#5.5.1%20哈夫曼树)来优化归并过程
	...
	* $k$ 叉最佳归并树最终一定是一棵严格的 $k$ 叉树，即树中只包含度为 $k$ 和度为 $0$ 的结点。设度为 k 的结点有 $n_{k}$ 个，度为 0 的结点有 $n_{0}$ 个，归并树总结点数 $=n$，则：初始归并段数量 + 虚段数量 $=n_{0}$ 且 $n=n_{0}+n_{k}$，除根结点外头上都连着一个分叉 $kn_{k}=n-1$，所以如果是严格 $k$ 叉树，$n_{k} = \dfrac{n_{0}-1}{k-1}$ 一定能整除
