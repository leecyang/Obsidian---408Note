...
* `Concat(&T, S1, S2)`：串的联接，用 `T` 返回由 `S1` 和 `S2` 联接而成的新串———可能会导致存储空间的扩展。如：执行Concat(&T, S, W)后，T = ‘iPhone 11 Pro Max?Pro’
* `SubString(&Sub, S, pos, len)`：求子串，用 `Sub` 返回串 `S` 的第 `pos` 个字符起长度为 `len` 的子串；
  如：执行SubString(&T, S, 4, 6)后，T = ‘one 11’
* `Index(S, T)`：定位操作，若主串 `S` 中存在与串 `T` 值相同的子串，则返回它在主串 `S` 中第一次出现的位置，否则返回 `0` 。如：执行Index(S, W)后，返回11
...
\begin{document}
	...
	\tikzstyle{every node}=[font=\huge, draw=black, very thick, rectangle, minimum width=1cm, minimum height=1cm]
		\foreach \char [count=\i] in {H,e,l,l,o, ,w,o,r,l,d,!, , , , } {
			\node[fill=gray!20] at (\i-1,4) {\char};
		}
		\node[fill=green!20] at(16,4){12};
		...
		\node at(12,2) {\textbackslash 0};
		
		\foreach \char [count=\i] in {H,e,l,l,o, ,w,o,r,l,d,!, , , } {
			\node[fill=gray!20] at (\i,1) {\char};
		}
		\node at(0,1) {};
		\node[fill=green!20] at(16,1){12};
	\end{tikzpicture}
\end{document}
```
...
HString S;
S.ch = (char *) malloc(MAXLEN * sizeof(char)); //基地址指针指向连续空间的起始位置
//malloc()需要手动free()
S.length = 0;
```
...
暴力解法，找到所有可能的子串，把这些子串与模式串一一对比，即 [`Index(S, T)`](#4.1.2%20串的基本操作)
...
* 在主串与模式串发生失配的字符之前，<u>前面的字符一定是和模式串一致的</u>，这些字符是已知的
* 根据模式串 `T`，求出 `next` 数组（只与模式串有关，与主串无关），利用 `next` 数组进行匹配，当匹配失败时，主串的指针 `i` 不再回溯！
* 即在模式串的第 `j` 个字符与主串发生失配时，模式串的指针跳到 `next[j]` 的位置，与主串当前位置的字符继续比较

> 当模式串第1个字符失配时，`j=next[1]=0`，再执行 `i++, j++`，即主串指针后移一位、模式串指针回到1，继续匹配下一个相邻子串
...
* 判断next数组所指的字符`T.ch[next[j]]`与原本失配的字符`T.ch[j]`是否相等：
	* 如果不相等，就保持原有状态；
	* <u>如果相等，则没必要再比较一次</u>，可以直接令`j=next[next[j]]`<!--比较了也没用，反正不相等-->


* 手算求`nextval`数组
	1. 求 `next` 数组
	2. 若`T.ch[next[j]] == T.ch[j]`，则令 `nextval[j] = nextval[next[j]]`，否则令 `nextval[j] = next[j]`
