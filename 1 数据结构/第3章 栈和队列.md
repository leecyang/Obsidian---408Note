...
* 缺点：顺序栈的存储空间大小不可变，容易出现上溢；可让两个栈共享同一片空间（共享栈）以提高空间利用率；
* 相关概念
...
**选择题**：[题目](1%20数据结构/王道数据结构.pdf#page=77&selection=606,0,611,1)、[答案](1%20数据结构/王道数据结构.pdf#page=80&selection=724,0,729,1)

**综合题**：[题目](1%20数据结构/王道数据结构.pdf#page=80&selection=357,0,362,2)、[答案](1%20数据结构/王道数据结构.pdf#page=84&selection=384,0,389,2)
...
A：不能！<u>会有假溢出，所以需要用 模运算 将存储空间 `{0,1,2,…,MaxSize-1}` 在逻辑上变成“环状”</u>——**循环队列**
* 初始：`Q.front = Q.rear = 0`;
...
定义一个变量 `size` 用于记录队列中当前元素的个数，初始化 `size = 0`，进队成功 `size++`，出队成功`size--`，根据 `size` 的值判断队满与队空
...
* 判满
	* 方案一：牺牲一个存储单元，`(Q.rear + 2) % MaxSize == Q.front`
...
* ​    <!--先进行中缀转后缀-->若扫描到运算符或界限符，则按照“中缀转后缀”相同的逻辑压入运算符栈 （期间也会弹出运算符，每当弹出一个运算符时 ^[即确定了生成的后缀表达式中的一个符号，然后去把这个符号进行运算，即后缀表达式求值]，就需要再弹出两个操作数栈的栈顶元素并执行相应运算，运算结果再压回操作数栈）
...
缺点：效率低，递归层数过多可能导致栈溢出；可能包含很多次重复计算
...
* 打印数据缓冲区，[SPOOLing技术](../3%20操作系统/第5章%20输入输出管理#5.2.4%20SPOOLing技术（假脱机技术）)中采用缓冲区队列
...
|题号|笔记|
|:----:|:----|
|[1](file://Z:/考研/计算机/01.数据结构习题精讲/03.第三章%20栈、队列和数组/03.3.3栈和队列的应用/01.3.3选择1.mp4)|进制转换和迷宫求解也是栈的典型应用|
...
**对称矩阵**（方阵），$a_{ij}=a_{ji}$
* 只存储主对角线和下（上）三角区的数据
* 可以使用一维数组存储这些元素，数组大小为 $1+2+\cdots+n=\dfrac{n(n+1)}{2}$
* 映射函数：矩阵下标→一维数组下标，
	* 当以行优先存储主对角线和下三角区数据时
		* 若 $i\geqslant j$，则 $a_{ij}$ 对应一维数组下标 $k_{ij}=[1+2+\cdots+(i-1)]+j-1=\dfrac{i(i-1)}{2}+j-1$ 
		* 若 $i<j$，则 $a_{ij}$ 对应一维数组下标 $k_{ij}=k_{ji}$ 
	* 当以列优先存储主对角线和下三角区数据时
		* 若 $i\geqslant j$，则 $a_{ij}$ 对应一维数组下标 $k_{ij}=[n+(n-1)+\cdots+(n-j+2)]+(i-j)=\dfrac{(2n-j+2)(j-1)}{2}+i-j$ 
		* 若 $i<j$，则 $a_{ij}$ 对应一维数组下标 $k_{ij}=k_{ji}$
...
* 只存储主对角线和下（上）三角区的数据，并在最后一个位置存储常量 $c$，数组大小为 $\dfrac{n(n+1)}{2}+1$ 
* 映射函数：矩阵下标→一维数组下标，
	* 当以行优先存储主对角线和下三角区数据时
		* 若 $i\geqslant j$，则 $a_{ij}$ 对应一维数组下标 $k_{ij}=[1+2+\cdots+(i-1)]+j-1=\dfrac{i(i-1)}{2}+j-1$ 
...
**三对角矩阵**（方阵）：当 $\left|i-j\right|>1$ 时，有 $a_{ij}=0$，即只需存储主对角线及其相邻的两条对角线上的元素
* 第一行和最后一行各有两个元素，其余各行各有三个元素，共 $3n-2$ 个元素
* 映射函数：以行优先存储时，若 $\left|i-j\right|\leqslant 1$，则 $a_{ij}$ 对应一维数组下标 $k_{ij}=2i+j-3$
...
稀疏矩阵：非零元素的个数远远少于矩阵元素的总个数
* 顺序存储——三元组`<行，列，值>`
* 链式存储——十字链表法
	* 向下域 `down`：指向同一列中的下一个非零元素
	* 向右域 `right`：指向同一行中的下一个非零元素
